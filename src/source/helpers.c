...
long double s21_pow(double base, double exp) {
  ...
  long double res = 1;
  if (s21_fabs(exp) <= POW_EPS || s21_fabs(base - 1.0) <= POW_EPS) {
    res = 1.0l;
  } else if (s21_fabs(base) <= POW_EPS && exp > 0) {
    res = 0.0l;
  } else if (is_nan(base) || is_nan(exp)) {
    res = S21_NAN;
  } else if (base < 0 && s21_fabs((double)s21_fmod(exp, 1.0)) > POW_EPS) {
    res = -S21_NAN;
  } else if (is_inf(base) && base < 0 &&
             s21_fabs((double)s21_fmod(exp, 2.0)) > POW_EPS) {
    res = -S21_INF;
  } else if (is_inf(base) && base < 0 && exp > 0) {
    res = S21_INF;
  } else if (is_inf(base) && base && exp < 0) {
    res = 0.0l;
    ...
    if (neg_base && s21_fabs((double)s21_fmod(exp, 2.0)) > POW_EPS) res = -res;
